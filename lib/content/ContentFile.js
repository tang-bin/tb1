...
class ContentFile {
    ...
    getFilePath() {
        ...
        if (fileUtil_1.fileUtil.isFile(p1))
            return p1;
        else if (fileUtil_1.fileUtil.isFolder(p1)) {
            const p2 = p1 + "/index.html";
            if (fileUtil_1.fileUtil.isFile(p2))
                return p2;
        }
        else if (!ext) {
            const p3 = p1 + "/index.html";
            if (fileUtil_1.fileUtil.isFile(p3))
                return p3;
        }
        return "";
    }
